...
function writeFragment(fragment) {
  ...
  logger.info({ params }, 'params in writeFragment()');
  // Create a PUT command to send to DynamoDB
  const command = new PutCommand(params);
  logger.info({ command }, 'command in writeFragment()');
...
async function listFragments(ownerId, expand = false) {
  ...
  const params = {
    ...
    ExpressionAttributeValues: { // to limit which attributes we want to be included in the returned result
      ':ownerId': ownerId,
    },
  };
...
// Delete a fragment's metadata from DynamoDB and data from S3 bucket. Returns a Promise
async function deleteFragment(ownerId, id) {
  // Delete a fragment's metadata from DynamoDB
  ...
  const command0 = new DeleteCommand(params0);

  try {
    await ddbDocClient.send(command0);
  } catch (err) {
